...
export default function NewScheduleModal({
  ...
  useEffect(() => {
    ...
    if (open && initialData) {
      setForm({
        ...initialData,
        alarmEnabled: initialData.alarmEnabled ?? false,
        alarmMinutes: initialData.alarmMinutes ?? 5,
      });
    }
  }, [open, initialData]);
  ...
  // Function to handle submit and save to localStorage
  const handleSubmit = () => {
    // Save schedule in localStorage
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("newSchedules");
      const schedules: ScheduleInput[] = saved ? JSON.parse(saved) : [];
      if (initialData) {
        const index = schedules.findIndex(
          (s) =>
            s.date === initialData.date &&
            s.startTime === initialData.startTime &&
            s.title === initialData.title
        );
        if (index > -1) {
      ...
      localStorage.setItem("newSchedules", JSON.stringify(schedules));
    }
      ...
      <div className="w-full max-w-lg rounded-xl bg-white p-6 space-y-4">
        ...
        <div className="rounded-lg border p-3 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">Alarm reminder</span>
            <input
              type="checkbox"
          ...
          {form.alarmEnabled && (
            <div className="flex items-center gap-3 text-sm">
              ...
              <select
                ...
                onChange={(e) => setForm({ ...form, alarmMinutes: Number(e.target.value) })}
              >
